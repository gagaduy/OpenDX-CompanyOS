...
export function InventoryTable({ items, canWrite, onView, onReceive, onAdjust }: {
  ...
  return <div className="tableFrame"><table className="inventoryTable" aria-label="Inventory stock levels"><thead><tr><th>Product</th><th>SKU</th><th>On hand</th><th>Reserved</th><th>Available</th><th>Status</th><th><span className="srOnly">Actions</span></th></tr></thead><tbody>{items.map((item) => <tr key={item.id}>
    ...
    <td data-label="SKU"><strong className="technicalText">{item.sku}</strong></td><td data-label="On hand">{item.onHand}</td><td data-label="Reserved">{item.reserved}</td><td data-label="Available"><strong>{item.available} available</strong></td>
