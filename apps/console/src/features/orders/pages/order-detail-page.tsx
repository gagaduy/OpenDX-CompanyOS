...
export function OrderDetailPage({ api, roles }: { readonly api: OrderOperationsApi; readonly roles: readonly StaffRole[] }) {
  const { orderId } = useParams(); const orderState = useOrder(api, orderId); const [saving, setSaving] = useState(false); const [mutationError, setMutationError] = useState<string>(); const [notice, setNotice] = useState<string>();
