...
export function ProductListPage({ api }: { readonly api: CatalogApi }) {
  ...
  return <section className="catalogWorkspace"><header className="workspaceHeader splitHeader"><div><p className="sectionKicker">Catalog</p><h1>Products</h1><p>{data?.totalItems ?? 0} products in NovaCommerce</p></div><Link className="primaryButton" to="/products/new"><Plus size={16} /> New product</Link></header>
    <div className="filterBar"><label className="searchControl"><Search size={15} /><span className="srOnly">Search products</span><input type="search" aria-label="Search products" placeholder="Search name, slug, or brand" value={query.query ?? ""} onChange={(event) => update("query", event.target.value)} /></label><label><span>Status</span><select aria-label="Status" value={query.status ?? ""} onChange={(event) => update("status", event.target.value)}><option value="">All statuses</option><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></label><label><span>Category</span><select aria-label="Category filter" value={query.categoryId ?? ""} onChange={(event) => update("categoryId", event.target.value)}><option value="">All categories</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label></div>
