...
export function ProductEditorPage({ api, roles = ["catalog_manager"] }: { readonly api: CatalogApi; readonly roles?: readonly StaffRole[] }) {
  ...
  async function saveProduct(input: Parameters<typeof editor.save>[0]) {
    const saved = await editor.save(input);
    if (actualId === undefined && saved !== undefined) {
