...
export function ProductTable({ products, onArchive }: { readonly products: readonly ProductListItem[]; readonly onArchive: (product: ProductListItem) => void }) {
  return <div className="tableFrame"><table className="productTable"><thead><tr><th>Product</th><th>Category</th><th>Variants</th><th>Price range</th><th>Status</th><th>Updated</th><th><span className="srOnly">Actions</span></th></tr></thead><tbody>{products.map((product) => <tr key={product.id}>
    <td><div className="productIdentity"><span className="thumbnail"><Image size={16} aria-hidden="true" /></span><span><Link to={`/products/${product.id}`}>{product.name}</Link><small>{product.brand ?? "No brand"} · {product.slug}</small></span></div></td>
