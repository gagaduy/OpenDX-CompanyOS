...
import { ProductStatus } from "./publication-panel";
...
function ProductThumbnail({ api, product }: { readonly api: CatalogApi; readonly product: ProductListItem }) {
  ...
  useEffect(() => {
    ...
    if (product.primaryMediaId !== undefined) void api.loadMediaPreview(product.id, product.primaryMediaId).then((loaded) => { objectUrl = loaded; if (active) setSource(loaded); else revoke(loaded); }).catch(() => undefined);
    return () => { active = false; if (objectUrl !== undefined) revoke(objectUrl); };
...
function revoke(source: string): void { if (source.startsWith("blob:") && typeof URL.revokeObjectURL === "function") URL.revokeObjectURL(source); }
function formatPrice(minimum?: number, maximum?: number): string { if (minimum === undefined) return "Not priced"; const format = (amount: number) => `₫${new Intl.NumberFormat("en-US").format(amount)}`; return maximum !== undefined && maximum !== minimum ? `${format(minimum)} – ${format(maximum)}` : format(minimum); }
