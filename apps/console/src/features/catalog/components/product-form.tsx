...
export function ProductForm({ product, categories, onSave }: { readonly product?: Product; readonly categories: readonly Category[]; readonly onSave: (input: ProductInput) => Promise<void> }) {
  ...
  return <form className="editorForm" onSubmit={(event) => void submit(event)}>
    <fieldset className="editorSection"><legend>Basic details</legend><div className="formGrid"><label>Name<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>Slug preview<input className="technicalText" value={slugPreview} onChange={(event) => setSlug(slugify(event.target.value))} /></label></div></fieldset>
    <fieldset className="editorSection"><legend>Classification</legend><div className="formGrid"><label>Category<select value={categoryId} onChange={(event) => setCategoryId(event.target.value)}><option value="">Select category</option>{categories.filter((item) => item.status === "active").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Brand<input value={brand} onChange={(event) => setBrand(event.target.value)} /></label></div></fieldset>
    <fieldset className="editorSection"><legend>Description and attributes</legend><label>Description<textarea rows={5} value={description} onChange={(event) => setDescription(event.target.value)} /></label><div className="attributeEditor"><p className="fieldLabel">Attributes</p>{rows.map((row, index) => <div className="attributeRow" key={row.id}><label>Attribute name {index + 1}<input aria-label={`Attribute name ${index + 1}`} value={row.name} onChange={(event) => setRows(rows.map((item) => item.id === row.id ? { ...item, name: event.target.value } : item))} /></label><label>Attribute value {index + 1}<input aria-label={`Attribute value ${index + 1}`} value={row.value} onChange={(event) => setRows(rows.map((item) => item.id === row.id ? { ...item, value: event.target.value } : item))} /></label></div>)}<button className="secondaryButton" type="button" onClick={() => setRows([...rows, { id: Math.max(...rows.map((row) => row.id)) + 1, name: "", value: "" }])}>Add attribute</button></div></fieldset>
