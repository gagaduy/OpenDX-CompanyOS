...
export function OrderDetailPage({ api }: { readonly api: OrderApi }) {
  ...
  return (
    <main id="main-content" className="content-page transaction-page order-detail-page">
      ...
      <div className="order-detail-layout">
        <section>
          ...
          <div className="order-detail-section"><h2>Tiến trình đơn hàng</h2><ol className="order-timeline transaction-timeline" aria-label="Tiến trình đơn hàng">{order.history.map((entry, index) => <li key={`${entry.occurredAt}-${index}`}><Check aria-hidden="true" /><div><strong>{orderStatusLabel(entry.newStatus)}</strong><small>{new Date(entry.occurredAt).toLocaleString("vi-VN")}</small></div></li>)}</ol></div>
        </section>
        <aside className="checkout-summary order-total-summary sticky-summary" aria-label="Tóm tắt đơn hàng"><h2>Tổng đơn hàng</h2><dl><div><dt>Tạm tính</dt><dd>{formatVnd(order.subtotalVnd)}</dd></div><div><dt>Ưu đãi</dt><dd>-{formatVnd(order.discountVnd)}</dd></div><div className="checkout-total"><dt>Tổng cộng</dt><dd>{formatVnd(order.totalVnd)}</dd></div></dl><p>Thuế đã bao gồm, không tách riêng.</p></aside>
      </div>
    </main>
  );
}
