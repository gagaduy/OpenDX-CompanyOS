...
export function AddressForm({
  ...
  readonly onSubmit: (value: AddressInput) => Promise<boolean>;
}) {
  ...
  return (
    <form className="address-form" onSubmit={(event) => void submit(event)}>
      <label>
        Người nhận
        <input required name="recipientName" maxLength={120} />
      </label>
      <label>
        Số điện thoại
        <input required name="phoneNumber" maxLength={30} />
      </label>
      <label className="wide">
        Địa chỉ
        <input required name="addressLine" maxLength={300} />
      </label>
      <label>
        Phường / xã
        <input required name="ward" maxLength={120} />
      </label>
      <label>
        Tỉnh / thành phố
        <input required name="provinceOrCity" maxLength={120} />
      </label>
      <label>
        Mã bưu chính
      ...
      <label className="wide">
        Ghi chú giao hàng
        <input name="deliveryNote" maxLength={500} />
      </label>
