...
suite("PostgresqlAgenticRepository", () => {
  ...
  it("serializes one idempotency key across competing files into a domain conflict", async () => {
    ...
    const first = transactions.run(async (session) => { const result = await repository.approveFilePreview(session, request(firstFileId)); entered(); await releaseFirst; return result; });
    await firstEntered;
    const second = transactions.run((session) => repository.approveFilePreview(session, request(secondFileId)));
    await waitForWaitingAdvisoryLock(pool); release();
