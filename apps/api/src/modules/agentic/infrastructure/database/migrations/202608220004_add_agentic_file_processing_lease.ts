...
export function up(pgm: MigrationBuilder): void { pgm.sql(`
  ALTER TABLE agentic_intake_files ADD COLUMN processing_claimed_at timestamptz;
  CREATE INDEX agentic_intake_files_processing_claim_idx ON agentic_intake_files(status,processing_claimed_at,created_at) WHERE status IN ('uploaded','scanning');
  ...
  BEGIN
    ...
    IF NOT ((OLD.status='uploaded' AND NEW.status IN ('scanning','rejected')) OR (OLD.status='scanning' AND NEW.status IN ('clean','rejected')) OR (OLD.status='clean' AND NEW.status IN ('previewed','rejected')) OR (OLD.status='previewed' AND NEW.status IN ('approved','rejected')) OR (OLD.status IN ('approved','rejected') AND NEW.status='deleted') OR (OLD.status=NEW.status AND (NEW.retention_claimed_at IS DISTINCT FROM OLD.retention_claimed_at OR NEW.object_deleted_at IS DISTINCT FROM OLD.object_deleted_at OR NEW.processing_claimed_at IS DISTINCT FROM OLD.processing_claimed_at))) THEN RAISE EXCEPTION 'Invalid agentic intake file transition' USING ERRCODE='P0001'; END IF;
  ...
  END; $f$;
`); }
export function down(pgm: MigrationBuilder): void { pgm.sql(`DROP INDEX IF EXISTS agentic_intake_files_processing_claim_idx; ALTER TABLE agentic_intake_files DROP COLUMN IF EXISTS processing_claimed_at;`); }
