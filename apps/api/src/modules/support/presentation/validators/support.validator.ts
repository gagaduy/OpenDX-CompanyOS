...
export const parseGenerateAiProposal=(v:unknown)=>z.object({prompt:z.string().trim().min(1).max(4000),ticketScope:z.enum(["all_actionable","customer_email_pending"]).default("all_actionable")}).parse(v);
