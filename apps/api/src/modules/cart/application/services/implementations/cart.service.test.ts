...
import type { StorefrontVariantReader, StorefrontVariantSummary } from "../../../../catalog";
...
describe("CartService", () => {
  it("does not create state for an empty read", async () => {
    ...
    expect(await fixture.service.get(owner)).toMatchObject({ status: "empty", ownerKind: "guest" });
  ...
  it("adds, aggregates, updates, and removes an authoritative cart line", async () => {
    ...
    expect(added).toMatchObject({ itemCount: 3, totalVnd: 75_000_000, requiresAction: false });
    expect(added.items[0]).toMatchObject({ quantity: 3, sku: variant.sku, availableQuantity: 5 });

    const updated = await fixture.service.updateItem(owner, added.items[0]!.id, 2);
    expect(updated.totalVnd).toBe(50_000_000);
    expect((await fixture.service.removeItem(owner, updated.items[0]!.id)).items).toEqual([]);
  });
  ...
  it("rejects unpublished variants and insufficient stock", async () => {
    ...
    await expect(fixture.service.addItem(owner, variant.variantId, 1)).rejects.toMatchObject({
      code: "PRODUCT_NOT_AVAILABLE",
    } satisfies Partial<CartApplicationError>);
    fixture.published = true;
    await expect(fixture.service.addItem(owner, variant.variantId, 6)).rejects.toMatchObject({
...
function createFixture() {
  ...
  let sequence = 0;
  const transactions = {
    run: (work: (session: DatabaseSession) => Promise<unknown>) => work({} as DatabaseSession),
    runReadOnly: (work: (session: DatabaseSession) => Promise<unknown>) => work({} as DatabaseSession),
  ...
  return {
    ...
    set published(value: boolean) { state.published = value; },
    get available() { return state.available; },
    set available(value: number) { state.available = value; },
  };
}
...
class MemoryCartRepository implements CartRepository {
  cart?: Cart;
  items: CartItem[] = [];
  async findActiveByOwner() { return this.cart; }
  async lockActiveByOwner() { return this.cart; }
  async create(_session: DatabaseSession, cart: Cart) { this.cart = cart; }
  async listItems() { return [...this.items]; }
  async findItem(_session: DatabaseSession, _cartId: string, itemId: string) { return this.items.find((item) => item.id === itemId); }
  async findItemByVariant(_session: DatabaseSession, _cartId: string, variantId: string) { return this.items.find((item) => item.variantId === variantId); }
  async createItem(_session: DatabaseSession, item: CartItem) { this.items.push(item); }
