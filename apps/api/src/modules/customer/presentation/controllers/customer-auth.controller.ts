...
export class CustomerAuthController {
  ...
  guest: RequestHandler = async (_req, res, next) => {
    try {
      ...
      setSessionCookie(res, this.cookies.guestName, issued.rawToken, issued.principal.expiresAt, this.cookies);
      this.setCsrf(res);
      res.status(201).json(successResponse("Guest session created", {
        kind: "guest", expiresAt: issued.principal.expiresAt,
      }));
  ...
  google: RequestHandler = async (req, res, next) => {
    try {
      const issued = await this.auth.loginWithGoogle(parseBody(googleSchema, req.body).credential);
      let guest;
      const guestToken = readCookie(req, this.cookies.guestName);
      if (guestToken !== undefined) {
        try { guest = await this.sessions.resolveGuest(guestToken); }
        catch { clearCookie(res, this.cookies.guestName, this.cookies); }
      }
      const cartResolution = this.cartResolver === undefined
        ? "not_required"
      ...
      setSessionCookie(res, this.cookies.customerName, issued.rawToken, issued.principal.expiresAt, this.cookies);
      this.setCsrf(res);
      res.json(successResponse("Customer signed in", {
        kind: "customer",
  ...
  session: RequestHandler = async (req, res, next) => {
    try {
      ...
      if (customer !== undefined) {
        ...
        setSessionCookie(res, this.cookies.customerName, issued.rawToken, issued.principal.expiresAt, this.cookies);
        this.setCsrf(res);
        res.json(successResponse("Customer session restored", {
          kind: "customer", customerId: issued.principal.customerId,
          email: issued.principal.email, expiresAt: issued.principal.expiresAt,
        }));
        return;
      }
      ...
      if (guest !== undefined) {
        ...
        this.setCsrf(res);
        res.json(successResponse("Guest session restored", { kind: "guest", expiresAt: principal.expiresAt }));
        return;
      }
