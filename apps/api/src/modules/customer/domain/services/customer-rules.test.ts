...
const address = (id: string, isDefault = false): CustomerAddress => ({
  id,
  customerId: "c",
  recipientName: "Duy",
...
describe("customer rules", () => {
  it("requires a verified provider identity and bounded profile", () => {
    ...
    ).toThrowError(expect.objectContaining({ code: "GOOGLE_TOKEN_INVALID" }));
    validateCustomerProfile("Duy", "0900000000");
    expect(() => validateCustomerProfile("x".repeat(121))).toThrowError(
      expect.objectContaining({ code: "INVALID_CUSTOMER_PROFILE" }),
    );
  });
  it("fixes guest and customer absolute expiry", () => {
    expect(sessionExpiresAt(NOW, GUEST_SESSION_TTL_MS)).toBe(
      "2026-08-12T00:00:00.000Z",
    );
    expect(sessionExpiresAt(NOW, CUSTOMER_SESSION_TTL_MS)).toBe(
      "2026-09-04T00:00:00.000Z",
    );
    expect(() => sessionExpiresAt(NOW, 1)).toThrowError(
      expect.objectContaining({ code: "INVALID_SESSION_EXPIRY" }),
    );
  });
  it("rejects expired and revoked sessions and rotates without extending expiry", () => {
    ...
    expect(() =>
      assertCustomerSessionActive({ ...session, revokedAt: NOW }, NOW),
    ).toThrowError(expect.objectContaining({ code: "SESSION_REVOKED" }));
    expect(() =>
      assertCustomerSessionActive(session, session.expiresAt),
  ...
  it("validates addresses and keeps one default", () => {
    ...
    expect(() =>
      validateAddress({ ...address("a"), recipientName: "" }),
    ).toThrowError(expect.objectContaining({ code: "INVALID_ADDRESS" }));
    expect(
      setDefaultAddress([address("a", true), address("b")], "b", NOW)
