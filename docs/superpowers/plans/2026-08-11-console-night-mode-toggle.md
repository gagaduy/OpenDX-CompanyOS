...
**Goal:** Add a persisted Console night-mode toggle with a light default and visibly darker night mode.
...
- Valid theme values are exactly `light` and `night`; invalid stored values fall back to `light`.
...
**Interfaces:**
- Produces DOM: `.consoleLayout[data-theme="light" | "night"]`
...
starts with `data-theme="light"`, clicks `Bật chế độ night`, verifies
`data-theme="night"` and persisted storage, then rerenders with an invalid
stored value and verifies fallback to `light`.
...
stored values to `light | night`, write changes back to storage, render
`data-theme`, and add the button near staff identity. Use `Moon` and `Sun` from
`lucide-react`.

- [ ] **Step 4: Add night CSS tokens**

In `globals.css`, add `.consoleLayout[data-theme="light"]` and `.consoleLayout[data-theme="night"]` with deeper `--canvas`, `--surface-*`, and
`--hairline` values while leaving `--primary` unchanged.

- [ ] **Step 5: Update changelog**

Add one `[Unreleased]` entry describing the Console light/night-mode toggle.
