...
test("uses idempotent separate database schema and namespace scripts", () => {
  ...
  assert.match(roles, /opendx_bootstrap_legacy/);
  assert.match(roles, /ALTER DATABASE opendx OWNER TO opendx_local/);
  assert.match(roles, /REVOKE CONNECT ON DATABASE opendx FROM PUBLIC/);
  assert.match(roles, /REVOKE CONNECT ON DATABASE postgres FROM PUBLIC/);
  assert.match(roles, /ALTER ROLE opendx_local WITH LOGIN NOSUPERUSER/);
  ...
  assert.match(create, /REVOKE CONNECT ON DATABASE temporal FROM opendx_local/);
  ...
  assert.match(namespace, /namespace describe --namespace opendx/);
  assert.match(namespace, /namespace create --namespace opendx --retention 168h/);
  assert.match(namespace, /operator cluster health/);
